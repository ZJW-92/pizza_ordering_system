    ...
    <el-row class="menu-main">
      ...
      <el-col :span="8" :offset="1" class="shopping-order about-content">
        <el-container direction="vertical">
          ...
          <el-row v-if="order.length<1" class="NoGoods">Tomt</el-row>
            ...
            <el-container direction="vertical">
              <el-form>
                <el-row class="menu-item" :key="Math.random()" v-for="option in order">
                  <el-col :span="12">{{option.goods}}<span>（{{option.size+'tum'}}）</span></el-col>
                  <el-col :span="5">{{option.price*option.num+' SEK'}}</el-col>
                ...
                <el-row class="goods-price-total" type="flex" align="middle">
                  <el-col :span="20">Summa：{{total+' SEK'}}</el-col>
                  <el-col :span="4" class="empty-goods"><img @click="emptyorder" title="Ta bort" src="../assets/empty.png"></el-col>
                </el-row>
                <el-row class="submit-btn">
                  <el-col><el-button @click="submitOrder">Beställa</el-button></el-col>
                </el-row>
              </el-form>
            </el-container>
          </el-container>
        ...
        submitOrder(){
          if (this.isLogin){
            ...
            this.axios.post('/orders.json',{
              ...
              goods:this.order,
              orderTime:orderT,
              totalPrice:this.total+'SEK'
            })
              .then(res=>{
                this.$message('Tack för din beställning');
                this.order='';
              });
          } else {
